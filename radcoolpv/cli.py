"""Command-line entry point: ``radcoolpv run case.yaml``."""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    run_p = sub.add_parser("run", help="Run a simulation from a YAML file.")
    run_p.add_argument("config", help="Path to the YAML file.")
    run_p.add_argument(
    ...
    run_p.add_argument(
        ...
        help="Parent folder for the timestamped results (default: results).",
    )
    run_p.add_argument("--no-plots", action="store_true", help="Skip the figures.")
    ...
    for cfg in configs:
        ...
        if args.print_config:
            ...
            continue
        try:
            pipeline.run(cfg, results_dir=args.results_dir, plots=not args.no_plots)
