...
def run(cfg, optics: OpticsResult, solar: SolarSpectrum) -> ThermalResult:
    ...
    emit_temp = t_amb + np.arange(pv._N_TEMP, dtype=float)
    ...
    # --- PV-free validation (Perrakis Fig. 2) ----------------------------- #
    if is_test:
    ...
    if cfg.thermal.equilibrium == "manual":
        ...
        for _ in range(50):
            ntp, cool = assemble(vmpp)
            equil_temp, equil_index = _zero_crossing(emit_temp, cool)
            vmpp_new = float(iv.volt[np.argmax(iv.cell_power[:, equil_index])])
    ...
    # Reporting at ambient (index 0) and equilibrium temperatures.
    mpp_amb = float(iv.cell_power[:, 0].max())
    mpp_equil = float(iv.cell_power[:, equil_index].max())
    voc_amb = pv.open_circuit_voltage(iv.current_dens[0, :], iv.volt)
    voc_equil = pv.open_circuit_voltage(iv.current_dens[equil_index, :], iv.volt)
    ...
    efficiency = iv.max_power_point / solar.total_am15
    efficiency_equil = float(efficiency[equil_index])
