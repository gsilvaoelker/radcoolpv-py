...
| [B — cooling power](notebooks/validation_b_cooling.ipynb) | no | The energy balance alone, on the paper's *measured* spectrum. At the stated *h* = 6 W/m²/K it gives 415.4 / 360.6 / 355.6 K against a reported 360 / 339 / 336; a fitted *h* = 12.54 reproduces the reported values. **The thermal model is not independently validated here.** |
...
Read them in order: A shows the optics are right, which is what places B's
sensitivity in the energy balance rather than in the solver, and C shows what
the whole chain predicts once both are in place.
...
wavelength-grid, and angular-grid checks, plus an independent reference. What
the worked example does and does not establish is set out in
